...
case class PieChartValue[T](value: T, label: String, color: Color, lighterColor: Color)(implicit
    ev: Numeric[T]
)

case class PieChart[T](
    baseId: String,
    title: String,
    entityName: String,
    valueUnit: String,
    values: Seq[PieChartValue[T]]
...
object PieChart {
  def apply[T](
      ...
      otherValuesLabel: String = "Other"
  )(implicit ev: Numeric[T]): PieChart[T] = {
    ...
    PieChart(
      ...
      valueUnit = valueUnit,
      values = minimalValuesSet.zipWithIndex.map { case ((value, label), index) =>
        PieChartValue(
          value,
          label,
          colors(index),
  ...
  private def minimalValuesSet[T](
      ...
      otherValuesLabel: String
  )(implicit num: Numeric[T]): Seq[(T, String)] = {
