...
object Slug {
  ...
  private def normalizedString(string: String): String = {
     ...
     CombiningDiacriticalMarks.
       replaceAllIn(nfdNormalizedString, ""). // Replace accented characters by their non-accented equivalents
       replaceAll("\\.", ""). // Remove dots (don't replace them by spaces)
       replaceAll("[^a-zA-Z0-9 ]", " "). // Keep only letters, digits and spaces
       replaceAll(" +", " "). // Replace multiple spaces by single spaces
